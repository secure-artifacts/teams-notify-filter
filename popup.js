...
const braveHintEl = document.getElementById("braveHint");
...
async function refreshMonitorStatus() {
  try {
    ...
    if (!res.hasTab) {
      monitorStatusEl.textContent = "请先打开 teams.live.com 或 teams.microsoft.com 并登录";
      return;
    }
    if (res.onLoginPage) {
      monitorStatusEl.textContent = "检测到 Teams 登录页 · 请在该标签完成登录";
      if (await detectBrave()) {
        monitorStatusEl.textContent += " · Brave 请关闭该站 Shields";
      }
      return;
    }
